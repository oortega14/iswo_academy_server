class AcademyConfiguration < ApplicationRecord
  ...
  validates :domain,
            ...
            uniqueness: { conditions: -> { where.not(id: self.id) } }
  validates :contact_email, format: { with: URI::MailTo::EMAIL_REGEXP }, allow_nil: true
  validates :color_palette, presence: true, inclusion: { in: %w[grey_breeze soft_spring dusty_rose calm_ocean golden_sand standard], message: 'paleta no válida' }
end
